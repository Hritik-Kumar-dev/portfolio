...
//   const [imgSrc, setImgSrc] = useState(Magz); // initial image link

// const onclick = () => {
//    console.log("changeImage1 called"); 
//     setImgSrc(sample); // new image link
//   };
...
//             <Myprolist onClick={onclick} tittle="Project 1"  desc="Description for project 1 is passed by using react props feature" stack_array={['React', 'JavaScript', 'CSS']} />
      ...
      <div className=''>
        <section className='Fourth-Section w-[100vw] h-[100vh] relative flex items-center justify-center'>
          ...
          <div className="master p-5 flex w-[100vw] h-[100vh] absolute justify-end m items-center z-40 ">
            ...
            <div className="list_container w-[40%] h-[90%]  overflow-y-hidden p-3  mt-2.5 mr-10 justify-center flex  z-80 backdrop-blur-md bg-cyan-200/10 rounded-[28px] shadow-xl items-center flex-col gap-3 poetsen-one-regular text-gray-300 text-lg ">
              {DataArray.map((item) => (
                // <div className='w-[50%] bg-amber-900 flex flex-col' key={item.id}>
                //     <h2 className='hover:cursor-pointer'  onClick={() => handleClick(item.videosrc)}>{item.title}</h2>
                //     <p>{item.desc}</p>
